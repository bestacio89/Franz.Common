...
{
  public class PollyAdvancedCircuitBreakerPipeline<TRequest, TResponse> : IPipeline<TRequest, TResponse>
  {
        ...
        CancellationToken cancellationToken = default)
    {
      ...
      {
        _logger.LogInformation("▶️ Executing {Request} [{CorrelationId}] with AdvancedCircuitBreaker {Policy}",
        ...
        try
        {
          ...
          stopwatch.Stop();
          _logger.LogInformation("✅ {Request} [{CorrelationId}] completed through AdvancedCircuitBreaker in {Elapsed}ms (policy {Policy})",
        ...
        {
          stopwatch.Stop();
          _logger.LogError(ex, "❌ {Request} [{CorrelationId}] faulted in AdvancedCircuitBreaker after {Elapsed}ms (policy {Policy})",
              requestName, correlationId, stopwatch.ElapsedMilliseconds, _policy.PolicyKey);
          throw;
        }
      }
