...
{
  public class PollyCircuitBreakerPipeline<TRequest, TResponse> : IPipeline<TRequest, TResponse>
  {
        ...
        CancellationToken cancellationToken = default)
    {
      ...
      {
        _logger.LogInformation("▶️ Executing {Request} [{CorrelationId}] with CircuitBreaker {Policy}",
        ...
        try
        {
          ...
          stopwatch.Stop();
          _logger.LogInformation("✅ {Request} [{CorrelationId}] passed CircuitBreaker in {Elapsed}ms (policy {Policy})",
        ...
        {
          stopwatch.Stop();
          _logger.LogError(ex, "❌ {Request} [{CorrelationId}] failed in CircuitBreaker after {Elapsed}ms (policy {Policy})",
              requestName, correlationId, stopwatch.ElapsedMilliseconds, _policy.PolicyKey);
          throw;
        }
      }
