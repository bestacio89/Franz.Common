...
{
  public class PollyTimeoutPipeline<TRequest, TResponse> : IPipeline<TRequest, TResponse>
  {
        ...
        CancellationToken cancellationToken = default)
    {
      ...
      {
        _logger.LogInformation("▶️ Executing {Request} [{CorrelationId}] with Timeout {Policy}",
        ...
        try
        {
          ...
          stopwatch.Stop();
          _logger.LogInformation("✅ {Request} [{CorrelationId}] completed within timeout in {Elapsed}ms (policy {Policy})",
        ...
        {
          stopwatch.Stop();
          _logger.LogError(ex, "❌ {Request} [{CorrelationId}] timed out/faulted after {Elapsed}ms (policy {Policy})",
              requestName, correlationId, stopwatch.ElapsedMilliseconds, _policy.PolicyKey);
          throw;
        }
      }
