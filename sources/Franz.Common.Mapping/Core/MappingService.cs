...
{
    private readonly ConcurrentDictionary<Type, object> _mapperCache = [];
    ...
    public TDestination Map<TSource, TDestination>(TSource source)
    {
        ...
        var mapper = GetOrCreateMapper<IMapper<TSource, TDestination>>(typeof(IMapper<TSource, TDestination>));
    ...
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
        var mapper = await GetOrCreateMapperAsync<IAsyncMapper<TSource, TDestination>>(typeof(IAsyncMapper<TSource, TDestination>), cancellationToken).ConfigureAwait(false);
    ...
    private async ValueTask<TMapper> GetOrCreateMapperAsync<TMapper>(Type mapperType, CancellationToken cancellationToken)
    {
        if (_mapperCache.TryGetValue(mapperType, out var cachedMapper))
        {
            return (TMapper)cachedMapper;
